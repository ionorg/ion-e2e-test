// Package e2e-test contains tools for end to end testing
package main
...
func main() {
	...
	rootCmd.PersistentFlags().StringVarP(&rid, "ion_room", "r", "test-pink-video", "room name to join")
	rootCmd.PersistentFlags().StringVarP(&video, "videofile", "f", "pink.video", "video file to join with")
	rootCmd.PersistentFlags().StringVarP(&username, "ion_username", "u", "test-pink-video", "username to join with")
